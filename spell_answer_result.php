 ...
 ****************************************************************/
	...
	include("db_name.php");
	$user = $_SESSION['sid'];
	$user_sql = mysql_query("SELECT * FROM $user_spell_db WHERE id = '$user' AND end = 0");
		...
		switch($user_row['Mode'])
		{
			...
			case "Group" :			
				...
				if($gr_total>0){ 
					...
					echo "題組:".$user_row['group']."<br>此題組遭刪除!";
				}
...
mysql_query("UPDATE $user_spell_db SET end=1 WHERE num = ".$user_num); //1 present this test has finished.
