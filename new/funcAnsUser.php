<?php
	...
	function countAnsNum ($uid) {
		...
		global $user_db;	
		$uquery = mysql_query("UPDATE `$user_db` SET `sum` = sum+1 WHERE `id` = '$uid' AND `end` = '0'") or die(mysql_error());
	...
	function countError ($uid, $qid) {
		...
		global $user_db;
		global $question_db;
		$uquery = mysql_query("UPDATE `$user_db` SET `false_num` = false_num+1 WHERE `id` = '$uid' AND `end` = '0'") or die(mysql_error());
